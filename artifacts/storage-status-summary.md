...
**Last updated:** 2026-09-01 (OSAC-4542 PR #563: rebased on latest origin/main [25 new commits], resolved proto merge conflicts. Both `option (cleanapi.file).private = true;` and `option (cleanapi.file).package = "osac.public.v1";` kept in volume_type.proto and volumes_service.proto — they work together. Regenerated .pb.go files, force-pushed to fork, new head 8bb0693b3. All 3 PRs (osac#563, EP#236, EP#237) remain bot-APPROVED, awaiting human `/lgtm` from danmanor.) — PRIOR: 2026-08-24 (PR #286 CI: stale-failure diagnosis — e2e had run 2026-08-13 with the branch 252 commits behind main, before OSAC-3714 [metering BSR→local proto] + OSAC-3972 [shared event schema, fixes `specversion: None`] landed. Rebased #286 on main + force-pushed; vmaas e2e now green, caas e2e + metering-unit still red. Root-caused the recurring osac-metering unit flake: `container.go:71 exec.CommandContext(ctx,...)` ties the Postgres container to BeforeSuite ctx; `postgres_test.go:47 defer cancel()` → SIGTERM Postgres → 57P03 on new connections mid-suite. From OSAC-3418; ~37% flake on main; untracked, team just /retests. LVMS decision PENDING: Roy leans Option A [contradicts doc's "A ruled out"] — needs transcript re-read + decision.) — PRIOR: 2026-08-18 (Volume PR #223 CLOSED, split into #339 reconciler + #340 controllers [OPEN, findings posted] + #341 remove-pvcRef [MERGED] + #342 tier-resolution [MERGED, closes Backend-empty gap]. LVMS-CSI options analysis complete: dev-only premise → B vs C, avoid A; doc in artifacts/lvms-csi/ [[project-osac-csi-next]]. VolumeSpec now tier/size/access_mode only. CSP admin storage UI reviewed for Elay: quota modeled on backend-association [aligns with WG], protocol NFS/Block mutual-exclusion good.) — PRIOR: 2026-08-14 (PR #286 OSAC-3985: nits addressed — BackendsClient nil check added to guard, requeueAfter log key dropped, error-path test added; all CI failures pre-existing. PR #327 OSAC-4041: bot PR analyzed — fullsend APPROVED after addressing nil-guard + test comment nits; needs our /lgtm + /approve. OSAC-4041 Bug ticket filed + linked to OSAC-917; StorageBackendsCreateRequest.object missing required=true deferred as systemic cross-repo issue. Graphify knowledge graph installed in osac repo: 99,990 nodes, 179,258 links; auto-fetch SessionStart hook pending Elior's PR.)  
**Owner:** Zoltan Szabo  
