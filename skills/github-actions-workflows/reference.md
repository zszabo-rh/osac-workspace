...
- **If a test exercises a real external resource** (a specific repo, tag,
  ...
  never disappearing or a network call always succeeding. (`osac-project/osac`
  now carries component-prefixed release tags per OSAC-3467, e.g.
  `fulfillment-service/v0.0.81` - if the default tag in `self-check.sh`
  starts resolving to "skip: couldn't resolve...", it's been pruned;
  bump it to whatever currently exists via `gh api
  repos/osac-project/osac/tags`.)
